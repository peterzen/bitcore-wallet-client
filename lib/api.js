...
var BASE_URL = 'https://bws.decred.org/bws/api';
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.network || 'dcrdlivenet');
};
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.network || 'dcrdlivenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || 'dcrdlivenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.contains(['dcrdlivenet', 'dcrdtestnet'], network));

  self._doGetRequest('/v1/feelevels/?network=' + (network || 'dcrdlivenet'), function(err, result) {
    if (err) return cb(err);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (!_.contains(['dcrdtestnet', 'dcrdlivenet'], network)) return cb(new Error('Invalid network'));
